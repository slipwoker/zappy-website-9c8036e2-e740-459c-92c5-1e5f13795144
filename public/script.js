...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (contactForm) {
        ...
        const inputs = [nameInput, emailInput, messageInput, phoneInput].filter(Boolean);
        inputs.forEach(function(input) {
            ...
            input.addEventListener('blur', function() {
                ...
                if (input === nameInput && value && value.length < 2) {
                    showError(input, 'الاسم يجب أن يكون حرفين على الأقل');
                } else if (input === emailInput && value && !validateEmail(value)) {
                    showError(input, 'يرجى إدخال بريد إلكتروني صحيح');
                } else if (input === messageInput && value && value.length < 10) {
                    showError(input, 'الرسالة يجب أن تكون 10 أحرف على الأقل');
                } else if (input === phoneInput && value && !validatePhone(value)) {
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '9c8036e2-e740-459c-92c5-1e5f13795144',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '9c8036e2-e740-459c-92c5-1e5f13795144';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '9c8036e2-e740-459c-92c5-1e5f13795144';
